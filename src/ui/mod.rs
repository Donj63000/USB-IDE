...
use crate::codex::{
    ...
    codex_install_argv, codex_install_prefix, codex_login_argv, codex_status_argv, extract_display_items,
    extract_status_code, node_executable, parse_tool_list, pip_install_argv, pyinstaller_available,
    pyinstaller_build_argv, pyinstaller_install_argv, resolve_in_path, tools_env, tools_install_prefix,
    DisplayKind,
};
use crate::fs::{detect_text_encoding, is_probably_binary, read_text_with_encoding};
use crate::process::{python_run_argv, stream_subprocess, windows_cmd_argv, ProcEventKind, ProcHandle};
...
fn build_tree(path: &Path) -> FileNode {
    ...
    FileNode { path: path.to_path_buf(), name, is_dir, children }
}

fn flatten_tree(node: &FileNode, depth: usize, expanded: &HashSet<PathBuf>, out: &mut Vec<TreeEntry>) {
    out.push(TreeEntry {
...
impl App {
    ...
    fn draw_tree(&mut self, f: &mut ratatui::Frame<'_>, area: Rect) {
        ...
        for entry in &self.tree.visible {
            ...
            let icon = if entry.is_dir { if is_expanded { "-" } else { "+" } } else { " " };
    ...
    fn draw_shell(&mut self, f: &mut ratatui::Frame<'_>, area: Rect) {
        ...
        let log = Paragraph::new(log_text).block(log_block).wrap(Wrap { trim: false });
    ...
    fn draw_codex(&mut self, f: &mut ratatui::Frame<'_>, area: Rect) {
        ...
        let log_text = self.render_log(&self.codex_log, chunks[1].height.saturating_sub(2) as usize);
        let log = Paragraph::new(log_text).block(log_block).wrap(Wrap { trim: false });
        f.render_widget(log, chunks[1]);
    }
    ...
    fn portable_env(&self, mut env_map: HashMap<String, String>) -> HashMap<String, String> {
        env_map.insert("PIP_CACHE_DIR".to_string(), self.root_dir.join("cache").join("pip").display().to_string());
        env_map.insert(
            "PYTHONPYCACHEPREFIX".to_string(),
            self.root_dir.join("cache").join("pycache").display().to_string(),
        );
        env_map.insert("TEMP".to_string(), self.root_dir.join("tmp").display().to_string());
        env_map.insert("TMP".to_string(), self.root_dir.join("tmp").display().to_string());
        env_map.insert("PYTHONNOUSERSITE".to_string(), "1".to_string());
        env_map.insert("CODEX_HOME".to_string(), self.root_dir.join("codex_home").display().to_string());
        env_map.insert("NPM_CONFIG_CACHE".to_string(), self.root_dir.join("cache").join("npm").display().to_string());
        env_map.insert("NPM_CONFIG_UPDATE_NOTIFIER".to_string(), "false".to_string());
        env_map
    }
    ...
    fn sanitize_codex_env(&self, env_map: &mut HashMap<String, String>) {
        ...
        let allow_custom_base = Self::truthy(std::env::var("USBIDE_CODEX_ALLOW_CUSTOM_BASE").ok().as_ref());
    ...
    fn codex_env(&self) -> HashMap<String, String> {
        ...
        env_map.entry("PYTHONUTF8".to_string()).or_insert_with(|| "1".to_string());
        env_map.entry("PYTHONIOENCODING".to_string()).or_insert_with(|| "utf-8".to_string());
    ...
    fn wheelhouse_path(&self) -> Option<PathBuf> {
        ...
        if wheelhouse.is_dir() { Some(wheelhouse) } else { None }
    }
    ...
    fn open_file(&mut self, path: PathBuf) {
        ...
        editor.set_block(Block::default().borders(Borders::ALL).title("Editeur"));
        self.editor = editor;
        self.current = Some(OpenFile { path, encoding, dirty: false });
        self.refresh_title();
    }

    fn action_save(&mut self) {
        let (path, encoding, dirty) = match self.current.as_ref() {
            Some(current) => (current.path.clone(), current.encoding.clone(), current.dirty),
            None => {
                self.log_issue("Aucun fichier ouvert.", "avertissement", "sauvegarde", LogTarget::Main);
                return;
            }
        };
        if !dirty {
    ...
    fn action_run(&mut self) {
        let (path, dirty) = match self.current.as_ref() {
            ...
            None => {
                self.log_issue("Ouvre un fichier .py.", "avertissement", "execution_python", LogTarget::Main);
                return;
            }
        };
        let is_py = path
        ...
        self.spawn_process(argv, env_map, "execution python", LogTarget::Main, ProcessKind::PythonRun);
    }
    ...
    fn action_toggle_codex_view(&mut self) {
        ...
        self.last_codex_message = None;
        let mode = if self.codex_compact_view { "Compact" } else { "Brut" };
    ...
    fn action_codex_login(&mut self) {
        ...
        if !codex_cli_available(Some(&self.root_dir), Some(&env_map)) {
            if !self.install_codex(false, LogTarget::Codex) {
                self.log_issue("Codex introuvable.", "erreur", "codex_login", LogTarget::Codex);
                return;
            }
        }
        ...
        let argv = codex_login_argv(Some(&self.root_dir), Some(&env_map), self.codex_device_auth_enabled());
        self.codex_log_ui(format!("$ {}", argv.join(" ")));
        self.spawn_process(argv, env_map, "login Codex", LogTarget::Codex, ProcessKind::CodexLogin);
    }

    fn action_codex_check(&mut self) {
        let env_map = self.codex_env();
        if !codex_cli_available(Some(&self.root_dir), Some(&env_map)) {
            self.log_issue("Codex non installe.", "avertissement", "codex_status", LogTarget::Codex);
            return;
        }
        ...
        self.codex_log_ui(format!(
            "node: {}",
            node_path.map(|p| p.display().to_string()).unwrap_or("absent".into())
        ));
        self.codex_log_ui(format!(
            "entrypoint: {}",
            entry_path.map(|p| p.display().to_string()).unwrap_or("absent".into())
        ));
        self.codex_log_ui(format!(
            "codex (PATH): {}",
            resolved.map(|p| p.display().to_string()).unwrap_or("absent".into())
        ));
        let argv = codex_status_argv(Some(&self.root_dir), Some(&env_map));
        self.codex_log_ui(format!("$ {}", argv.join(" ")));
        self.spawn_process(argv, env_map, "verification Codex", LogTarget::Codex, ProcessKind::CodexStatus);
    }

    fn action_dev_tools(&mut self) {
        let raw = std::env::var("USBIDE_DEV_TOOLS").unwrap_or_else(|_| "ruff black mypy pytest".to_string());
        let tools = parse_tool_list(&raw);
        if tools.is_empty() {
            self.log_issue("Liste outils vide.", "avertissement", "outils_dev", LogTarget::Main);
            return;
        }
        ...
        let argv = match pip_install_argv(&prefix, &tools, wheelhouse.as_deref(), wheelhouse.is_some()) {
            Ok(argv) => argv,
            Err(err) => {
                self.log_issue(&format!("Impossible d'installer outils: {err}"), "erreur", "outils_dev", LogTarget::Main);
                return;
            }
        };
        self.log_ui(format!("$ {}", argv.join(" ")));
        self.spawn_process(argv, env_map, "installation outils dev", LogTarget::Main, ProcessKind::DevTools);
    }
    ...
    fn action_build_exe(&mut self) {
        let (path, dirty) = match self.current.as_ref() {
            Some(current) => (current.path.clone(), current.dirty),
            None => {
                self.log_issue("Ouvre un fichier .py.", "avertissement", "build_exe", LogTarget::Main);
                return;
            }
        };
        let is_py = path
        ...
        if !pyinstaller_available(Some(&self.root_dir), Some(&env_map)) {
            if !self.install_pyinstaller(false) {
                self.log_issue("PyInstaller indisponible.", "erreur", "build_exe", LogTarget::Main);
                return;
            }
        }
        let dist_dir = self.root_dir.join("dist");
        let _ = fs::create_dir_all(&dist_dir);
        let argv = match pyinstaller_build_argv(&path, &dist_dir, false, Some(&self.root_dir.join("tmp")), None) {
            Ok(argv) => argv,
            Err(err) => {
                self.log_issue(&format!("Erreur build: {err}"), "erreur", "build_exe", LogTarget::Main);
                return;
            }
        };
        self.log_ui(format!("$ {}", argv.join(" ")));
        self.spawn_process(argv, env_map, "construction exe", LogTarget::Main, ProcessKind::PyInstallerBuild);
    }
    ...
    fn install_pyinstaller(&mut self, force: bool) -> bool {
        ...
        let _ = fs::create_dir_all(&prefix);
        let wheelhouse = self.wheelhouse_path();
        let argv = match pyinstaller_install_argv(&prefix, wheelhouse.as_deref(), wheelhouse.is_some()) {
            Ok(argv) => argv,
            Err(err) => {
                self.log_issue(&format!("Impossible d'installer PyInstaller: {err}"), "erreur", "installation_pyinstaller", LogTarget::Main);
                return false;
            }
        };
        self.log_ui(format!("Installation PyInstaller (bin={})", prefix.display()));
        self.log_ui(format!("$ {}", argv.join(" ")));
        self.spawn_process(argv, env_map, "installation PyInstaller", LogTarget::Main, ProcessKind::PyInstallerInstall);
        true
    }
    ...
    fn install_codex(&mut self, force: bool, target: LogTarget) -> bool {
        ...
        if !force && !self.codex_auto_install_enabled() {
            self.log_issue("Auto-install Codex desactive.", "avertissement", "installation_codex", target);
            return false;
        }
        self.codex_install_attempted = true;
        let package = std::env::var("USBIDE_CODEX_NPM_PACKAGE").unwrap_or_else(|_| "@openai/codex".to_string());
        let prefix = codex_install_prefix(&self.root_dir);
        let _ = fs::create_dir_all(&prefix);
        let argv = match codex_install_argv(&self.root_dir, &prefix, &package) {
            Ok(argv) => argv,
            Err(err) => {
                self.log_issue(&format!("Impossible d'installer Codex: {err}"), "erreur", "installation_codex", target);
                return false;
            }
        };
        self.push_log(target, format!("Installation Codex package={package} prefix={}", prefix.display()), Style::default());
        self.push_log(target, format!("$ {}", argv.join(" ")), Style::default());
        self.spawn_process(argv, env_map, "installation Codex", target, ProcessKind::CodexInstall);
        true
    }
    ...
    fn run_shell(&mut self, cmd: String) {
        ...
        let env_map = self.portable_env(std::env::vars().collect());
        self.spawn_process(argv, env_map, "commande shell", LogTarget::Main, ProcessKind::Shell);
    }
    ...
    fn run_codex(&mut self, prompt: String) {
        ...
        if !codex_cli_available(Some(&self.root_dir), Some(&env_map)) {
            ...
            if installed {
                ...
                self.log_issue("Codex indisponible. (Ctrl+I pour installer)", "erreur", "codex_exec", LogTarget::Codex);
            }
            return;
        }
        ...
        self.pending_codex_prompt = Some(prompt);
        let argv = codex_status_argv(Some(&self.root_dir), Some(&env_map));
        self.spawn_process(argv, env_map, "codex_status", LogTarget::Codex, ProcessKind::CodexStatus);
    }
        ...
        match proc.kind {
            ProcessKind::CodexStatus => {
                if let Some(prompt) = self.pending_codex_prompt.take() {
                    if code == Some(0) {
                        let env_map = self.codex_env();
                        match codex_exec_argv(&prompt, Some(&self.root_dir), Some(&env_map), true, None) {
                            Ok(argv) => {
                                if !self.codex_compact_view {
                                    self.codex_log_ui(format!("$ {}", argv.join(" ")));
                                }
                                self.spawn_process(argv, env_map, "codex_exec", LogTarget::Codex, ProcessKind::CodexExec);
                            }
                            Err(err) => {
                                self.log_issue(&format!("Erreur Codex: {err}"), "erreur", "codex_exec", LogTarget::Codex);
                            }
                        }
                    } else {
    ...
    fn handle_codex_line(&mut self, line: &str) {
        ...
        if self.codex_compact_view {
            if matches!(event_type, "response.output_text.delta" | "response.output_text") {
                let delta = value
                    .get("delta")
        ...
        if event_type == "error" {
            let msg = value.get("message").and_then(serde_json::Value::as_str).unwrap_or("");
            if self.codex_compact_view {
    ...
    fn codex_log_entry(&mut self, msg: &str, label: &str, kind: &str) {
        ...
        self.push_log(LogTarget::Codex, label.to_string(), Style::default().add_modifier(Modifier::BOLD));
...
mod tests {
    ...
    fn portable_env_defauts() {
        ...
        assert_eq!(env.get("PIP_CACHE_DIR").unwrap(), &dir.path().join("cache").join("pip").display().to_string());
        assert_eq!(env.get("PYTHONPYCACHEPREFIX").unwrap(), &dir.path().join("cache").join("pycache").display().to_string());
        assert_eq!(env.get("TEMP").unwrap(), &dir.path().join("tmp").display().to_string());
        assert_eq!(env.get("TMP").unwrap(), &dir.path().join("tmp").display().to_string());
        assert_eq!(env.get("PYTHONNOUSERSITE").unwrap(), "1");
        assert_eq!(env.get("CODEX_HOME").unwrap(), &dir.path().join("codex_home").display().to_string());
        assert_eq!(env.get("NPM_CONFIG_CACHE").unwrap(), &dir.path().join("cache").join("npm").display().to_string());
    ...
    fn sanitize_codex_env_supprime() {
        ...
        with_env_lock(|| {
            let mut env = HashMap::from([
                ...
                ("OPENAI_BASE_URL".to_string(), "https://example.com".to_string()),
            ]);
